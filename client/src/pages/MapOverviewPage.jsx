...
export default function MapOverviewPage() {
  ...
  const { lastEvent } = useSocket(facilityId)
  ...
  useEffect(() => {
    if (!lastEvent || lastEvent.type !== 'slot:updated') return
    const update = lastEvent.payload
    setSlots((prev) =>
      prev.map((slot) =>
    ...
    setFlashSlotId(update.slotId)
    setTimeout(() => setFlashSlotId(''), 340)
    if (facilityId) {
      refreshSummary(facilityId).catch(() => {})
    }
  }, [lastEvent, facilityId, refreshSummary])
