...
function ChannelToggle({ icon, title, subtitle, enabled, onToggle, actionLabel }) {
  return (
    <div className="glass-card flex items-center justify-between px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onToggle}
          className={`h-6 w-12 rounded-full p-1 transition ${enabled ? 'bg-brand-green/30' : 'bg-dark-border'}`}
        >
          <span
            className={`block h-4 w-4 rounded-full bg-white transition-transform ${enabled ? 'translate-x-6' : 'translate-x-0'}`}
          />
        </button>
