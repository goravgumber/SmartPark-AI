          ...
          {aiRows.map((row) => (
            <div key={row.id} className="glass-card flex items-center justify-between px-3 py-2 text-sm">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => addToast({ type: 'warning', title: `${row.zone} suggestion applied` })} className="rounded bg-brand-amber/20 px-2 py-1 text-xs text-brand-amber">Apply</button>
                <button type="button" className="rounded border border-dark-border px-2 py-1 text-xs text-slate-400">Ignore</button>
              </div>
            </div>
          ))}
        </div>
